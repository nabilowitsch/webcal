...
function openDetailModal(idx) {
    ...
    const cal = state.calendars.find(c => c.href === ev.calendarHref);
    ...
    let dateStr;
    if (ev.allDay) {
        // end is exclusive per RFC 5545 — show last inclusive day
        const dispEnd = new Date(end);
        dispEnd.setUTCDate(dispEnd.getUTCDate() - 1);
        const startLabel = start.toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' });
        const endLabel   = dispEnd.toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' });
        dateStr = start.toISOString().slice(0, 10) === dispEnd.toISOString().slice(0, 10)
            ? startLabel
            : `${startLabel} – ${endLabel}`;
    } else {
        ...
        if (sameDay) {
            dateStr = `${start.toLocaleDateString(undefined, opts)}, ${start.toLocaleTimeString(undefined, timeOpts)} – ${end.toLocaleTimeString(undefined, timeOpts)}`;
        } else {
            dateStr = `${start.toLocaleString(undefined, { ...opts, ...timeOpts })} – ${end.toLocaleString(undefined, { ...opts, ...timeOpts })}`;
        }
    }

    const row = (icon, content) => `
        <div class="flex gap-3 items-start text-sm">
            <span class="mt-0.5 shrink-0 text-gray-400">${icon}</span>
            <span class="text-gray-700">${content}</span>
        </div>`;

    const clockIcon = `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>`;
    const calIcon   = `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>`;
    const pinIcon   = `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>`;
    const noteIcon  = `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>`;

    const calContent = cal
        ? `<span class="inline-flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full shrink-0" style="background-color:${cal.color}"></span>${esc(cal.name)}</span>`
        : '';

    let rows = row(clockIcon, esc(dateStr));
    if (cal)          rows += row(calIcon,  calContent);
    if (ev.location)  rows += row(pinIcon,  esc(ev.location));
    if (ev.description) rows += row(noteIcon, `<span class="whitespace-pre-wrap">${esc(ev.description)}</span>`);

    document.getElementById('detail-title').textContent = ev.summary;
...
function openEditModal(idx) {
    if (window.__IS_PUBLIC) { openDetailModal(idx); return; }
    const ev = evReg[idx];
    if (!ev) return;
    _editingEvent = ev;
